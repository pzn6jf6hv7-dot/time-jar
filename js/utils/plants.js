// 妞嶇墿缁樺埗閫昏緫 - Canvas API

// 妞嶇墿缁樺埗涓诲嚱鏁?function drawPlant(ctx, width, height, plantType, stage, color) {
...
function getStageLabel(stage) {
  const labels = ['绉嶅瓙', '鍙戣娊', '骞艰嫍', '鎴愰暱', '寮€鑺?, '缁撴灉'];
  return labels[stage] || '';
}

// 鑾峰彇闃舵 emoji
function getStageEmoji(stage) {
  const emojis = ['馃尠', '馃尡', '馃尶', '馃', '馃尯', '馃専'];
  return emojis[stage] || '';
}
